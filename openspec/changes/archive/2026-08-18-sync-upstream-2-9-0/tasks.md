...
- [x] 5.5 Live smoke: superseded security-hardened pass on 2026-08-18; `doc/validation/2026-08-18-antigravity-smoke.md` records `google-antigravity/gemini-3.7-flash`, a no-input deterministic payload-only tool policy (no approval or built-in tools), one `context-prune-summary`, `context-prune-flush-metrics` `message-end`/`summarized`, no fallback warning, and copied credential removal before retained-artifact inspection.
