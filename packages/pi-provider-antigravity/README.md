...
The live validator verifies that every expected wire ID is present as a non-internal discovery entry, captures the outgoing model ID through Pi's supported `onPayload` callback, and requires a completed primary-endpoint HTTP 200 response.
It runs one request attempt by default to avoid accidentally exercising fallback paths; set `ANTIGRAVITY_LIVE_ATTEMPTS=2` or `3` only for expected transient retries.
...
`/login` will prompt for authorization.
Complete the consent screen in the browser, return to the terminal.
If the callback fails or you cancel, you can paste the redirect URL into the terminal prompt and the extension will pick it up from there.
