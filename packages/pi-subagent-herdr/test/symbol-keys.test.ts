...
describe("symbol key rename", () => {
	it("uses pi-subagent-herdr/* Symbol.for keys and no pi-subagents/ keys", () => {
		...
		assert.match(index, /Symbol\.for\("pi-subagent-herdr\/widget-interval"\)/);
		assert.match(index, /Symbol\.for\("pi-subagent-herdr\/status-interval"\)/);
		assert.match(index, /Symbol\.for\("pi-subagent-herdr\/runtime"\)/);
