...
function validateActivity(value: unknown, expectedRunningChildId: string): ActivityReadResult {
	...
	const validationError = [
		...
		validateOptionalInteger(object, "toolCount"),
		validateOptionalFiniteNumber(object, "contextTokens"),
		validateOptionalFiniteNumber(object, "contextWindow"),
		validateOptionalFiniteNumber(object, "contextPercent"),
		validateOptionalInteger(object, "compactionCount"),
...
export function createSubagentActivityRecorder(params: {
	...
	const activityFile: string = candidateActivityFile;
	...
	return {
		...
		compaction() {
			// Keep latestEvent on the pre-existing vocabulary so an older version-1
			// reader accepts the sidecar and simply ignores the additive counters.
			record(
		...
		contextUsage(tokens, contextWindow, percent) {
			record(
				activity.latestEvent,
				(current) => {
					if (tokens != null) current.contextTokens = tokens;
