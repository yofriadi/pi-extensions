...
export function observePaneInspection(
	...
	const hasWorked =
		lifecycle.hasWorked || agentStatus === "working" || agentStatus === "blocked" || agentStatus === "done";
...
export function formatLifecycleTransitionLine(
	...
	const duration =
		projection.stateDurationSince == null ? "" : ` ${formatElapsed(now - projection.stateDurationSince)}`;
