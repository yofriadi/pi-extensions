...
Current TypeScript rejects the former file-argument G4 command when it discovers the parent monorepo `tsconfig.json` (TS5112). The local layer therefore needs an explicit package-local project configuration and package-owned current compiler/type dependencies that work after a standalone fork clone.
...
Instead: the local layer is `git diff 1d040e68 <clean-tip>:packages/pi-condense` (after Phase 0 commits everything), sliced into a tooling bootstrap plus three thematic commits applied in the fork on top of `125147c1`:

0. `chore(tooling): configure standalone TypeScript 7 checks` — `tsconfig.json` scoped to `index.ts` with the former G4 compiler options, plus `package.json` `typecheck` script and exact development dependencies `typescript` 7.0.2 and `@types/node` 22.19.19. This bootstrap lands first so every later slice runs G4 in project mode with the package-owned compiler, independent of a parent monorepo configuration or global installation.
...
3. `chore: local identity and scaffolding` — `package.json` identity + version `2.9.1` (the TypeScript script and development dependencies already landed in the tooling bootstrap), the complete `openspec/` tree (`config.yaml`, `specs/`, archived pacing change, active `summarizer-fallback-model`, and this `sync-upstream-2-9-0` change), `.pi/`, `AGENTS.md`/`AGENTS.core.md` deletions plus removal of the CI step + `check:agents-core` script + `scripts/check-agents-core.mjs` they break, CHANGELOG renumber (local `## [2.9.0]` → `Unreleased`, see Decision 3 table), README/PRUNING/doc entries.

Slices are ordered by dependency: the tooling bootstrap enables G4; `summarizer.ts` (slice ②) imports `RATE_LIMIT_*`/`RateLimitGate`/etc. from `./summarizer-pacing.js` (slice ①) and reads the `pacing` option seam in `types.ts` (slice ①), so pacing must land before dispatch. The harness port is fused into slice ② because the fork base `125147c1` already contains the unported `reload-rearm.integration.test.ts`, which fails G3 the moment dispatch lands (the fake's missing `getProvider` throws a TypeError that the summarizer buckets as transient). Each slice is applied with `git apply -3` (3-way against the upstream base), gated (tests + typecheck + ANTIGRAVITY greps), then committed. Bisectability matters more than perfectly atomic slices.
...
| `package.json` | **merge** | Take upstream version bumps and dependency changes; keep local `name`, `publishConfig`, and the standalone `typecheck` script plus TypeScript 7 development dependencies. |
...
3. G4 typecheck: `bun run typecheck`, which runs the package-owned TypeScript 7.0.2 compiler in project mode through `tsconfig.json` and does not inherit a parent monorepo configuration.
...
- **Phase 2 — layer reconstruction**: apply the TypeScript tooling bootstrap, then the three thematic slices from the monorepo diff onto `125147c1` with `git apply -3`; resolve per Decision 3; G4 green after each commit; commit.
