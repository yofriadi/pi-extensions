...
- [ ] 2.2 Clone `jjuraszek/pi-condense` to `~/Developer/oss/pi-condense`; remotes `origin` → fork, `upstream` → `jjuraszek/pi-condense`; run `bun install` in the fork, then after the tooling bootstrap rerun it and verify `bun run typecheck` resolves the declared package-owned TypeScript 7.0.2 and Node 22 types in the fresh clone
...
- [ ] 3.2 Slice ⓪ `chore(tooling): configure standalone TypeScript 7 checks` — `tsconfig.json` scoped to the `index.ts` graph, plus `package.json` `typecheck` script and exact `typescript` 7.0.2 / `@types/node` 22.19.19 development dependencies; apply with `git apply -3`, run `bun install`, gate on G4 (`bun run typecheck`), and commit. This bootstrap MUST precede every feature slice.
...
- [ ] 3.7 Full gates G1–G6 on `local/main` tip; verify version `2.9.1`.
