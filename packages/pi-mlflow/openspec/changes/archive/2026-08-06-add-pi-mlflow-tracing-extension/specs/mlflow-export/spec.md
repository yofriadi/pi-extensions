...
The system SHALL read its connection configuration (tracking URI, experiment name, content-capture mode) from a local configuration file, with a documented default tracking URI pointing at a typical local MLflow server.

#### Scenario: Default tracking URI is used when unspecified

- **WHEN** the configuration file does not specify a tracking URI
- **THEN** the extension defaults to a local MLflow server address (`http://localhost:5000`)
