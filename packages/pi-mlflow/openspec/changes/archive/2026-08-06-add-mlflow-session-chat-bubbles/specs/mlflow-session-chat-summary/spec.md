...
Project documentation for the extension SHALL state that MLflow Chat Sessions conversation (bubbles / turn Inputs and Outputs) requires `captureContent: true`, and that with the default `false` the Sessions turn body remains empty by design while structural tracing still works.
The README, `pi-mlflow.example.json` guidance, and the `/mlflow` status or command help surface SHALL each carry that information.
...
- **THEN** they are informed that enabling `captureContent` is required for session conversation bubbles

#### Scenario: Example config documents the requirement

- **WHEN** an operator reads `pi-mlflow.example.json` (or its inline comments / adjacent documented keys)
